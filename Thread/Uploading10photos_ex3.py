...
for URL in img_urls:
    i+=1
    # DownloadImage(URL,f"photo{i}")      #without threading
    th1 = threading.Thread(target=DownloadImage, args=(URL,f"photo{i}", ))
    th1.start()
th1.join()
stop = time.perf_counter()
print(f"Cycle time of all  : {stop-start}")
#Cycle time without threading: 15.90197 sec
#Cycle time with threading: 1.6592621 sec